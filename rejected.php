...
// Fetch rejected PDFs
$stmt = $conn->prepare("SELECT pdf_submissions.*, users.username FROM pdf_submissions JOIN users ON pdf_submissions.user_id = users.id WHERE status = 'unaccepted' ORDER BY submission_date DESC");
$stmt->execute();
...
<head>
    ...
    <title>Rejected PDF Submissions</title>
    <style>
...
<body>
    <header>
        <h1>Rejected PDF Submissions</h1>
        <nav>
